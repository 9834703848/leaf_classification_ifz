...
class ProposalLayer(K.layers.Layer):
...
def log2_graph(x):
    ...
    denominator = tf.math.log(tf.constant(2.0, dtype=numerator.dtype))
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    # Classifier head
    shared = Lambda(lambda t: tf.cast(t, tf.float32))(shared)
   
    mrcnn_class_logits = KL.TimeDistributed(KL.Dense(num_classes),
                                            name='mrcnn_class_logits')(shared)
    mrcnn_class_logits = Lambda(lambda t: tf.cast(t, tf.float32))(mrcnn_class_logits)
    mrcnn_probs = KL.TimeDistributed(KL.Activation("softmax"),
                                     name="mrcnn_class")(mrcnn_class_logits)
    mrcnn_probs = Lambda(lambda t: tf.cast(t, tf.float32))(mrcnn_probs)
...
def rpn_class_loss_graph(rpn_match, rpn_class_logits):
    ...
    # Crossentropy loss
    loss = tf.sparse_categorical_crossentropy(target=anchor_class,
...
def mrcnn_bbox_loss_graph(target_bbox, target_class_ids, pred_bbox):
    ...
    pred_bbox = tf.reshape(pred_bbox, (-1, tf.int_shape(pred_bbox)[2], 4))
...
def mrcnn_mask_loss_graph(target_masks, target_class_ids, pred_masks):
    ...
    loss = tf.switch(tf.size(y_true) > 0,
                    tf.binary_crossentropy(target=y_true, output=y_pred),
                    tf.constant(0.0))
    loss = tf.mean(loss)
    return loss
...
class MaskRCNN():
    ...
    def print_loaded_weights_structure(self,filepath):
     import h5py
     with h5py.File(filepath, mode='r') as f:
        ...
        for layer_name in f.keys():
            ...
            for weight_name in f[layer_name].keys():
                ...
                print(f"\t\tShape: {f[layer_name][weight_name].shape}")
    def load_weights(self, filepath, by_name=False, exclude=None):
        ...
        #self.print_loaded_weights_structure(filepath)
        with h5py.File(filepath, mode='r') as f:
            if 'layer_names' not in f.attrs and 'model_weights' in f:
                f = f['model_weights']
    ...
    def detect(self, images, verbose=0):
        ...
        molded_images = tf.cast(molded_images, tf.float32)
        image_metas = tf.cast(image_metas, tf.float32)
    ...
    def run_graph(self, images, outputs, image_metas=None):
        ...
        inputs = model.inputs
        if model.uses_learning_phase and not isinstance(tf.learning_phase(), int):
            inputs += [tf.learning_phase()]
